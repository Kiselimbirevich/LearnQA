...
@pytest.mark.parametrize('ua, expected_value', [
...
def test_ua(ua, expected_value):
    req = requests.get('https://playground.learnqa.ru/ajax/api/user_agent_check', headers = {"User-Agent": ua})
    data = req.json()
    assert data["platform"] == expected_value["platform"], f"Вместо {expected_value['platform']} получили {data['platform']}"
    assert data["browser"] == expected_value["browser"], f"Вместо {expected_value['browser']} получили {data['browser']}"
    assert data["device"] == expected_value["device"], f"Вместо {expected_value['device']} получили {data['device']}"